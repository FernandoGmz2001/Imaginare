...
export default function Generator() {
  ...
  console.log("hola estoy mostrando el imageObj")
      ...
      <div>
        <form className="relative" onSubmit={handleSubmit}>
          ...
          <button
            ...
            disabled={isLoading}
          >
            {isLoading ? <LuLoader2 className="animate-spin" /> : <FaArrowUp />}
          </button>
        </form>
      </div>
      {error && <p className="text-red-500">{error}</p>}
      {imageObj.url && <img src={imageObj.url} alt="Generated NFT" className="mt-4" />}
    </>
  );
}
