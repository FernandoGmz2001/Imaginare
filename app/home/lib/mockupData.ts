export const mockup = [
  {
    status:"Venta no iniciada",
    title: "Dreamcore Destiny",
    autor: "Nygilia",
    description: "Esta es la descripcion de un dragon espiritual que fue creado en noviembre del 2024 para poder ser utilizado como una demostracion en el diseño para la materia de Desarrollo de aplicaciones descentralizadas, permitiendo pasar la materia con una alta calificacion, tomando en cuenta el diseño y además la cantidad de contratos que serán incorporados a este.",
    nftUrl: "/images/BackgroundOffer_2.png",
    price: 0.0010,
  },
  {
    status:"Venta no iniciada",
    title: "Ko-Bah",
    autor: "Maxwell",
    description: "Ko-Bah represents the anti-hero character in the limited release “Twin Fist” drop. One of two limited edition poster sized collectibles. This piece was digitally illustrated with mixed media and hand placed digital impasto brushstrokes.",
    nftUrl: "/images/BackgroundOffer_3.png",
    price: 0.00500,
  },
  {
    status:"Venta no iniciada",
    title: "Spiritual Dragon Multicolor",
    autor: "Fernando Gomez",
    description: "Esta es la descripcion de un dragon espiritual que fue creado en noviembre del 2024 para poder ser utilizado como una demostracion en el diseño para la materia de Desarrollo de aplicaciones descentralizadas, permitiendo pasar la materia con una alta calificacion, tomando en cuenta el diseño y además la cantidad de contratos que serán incorporados a este.",
    nftUrl: "/images/hero-dragon.png",
    price: 0.0004,
  },
]

